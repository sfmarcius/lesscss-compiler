...
public class CompileMojo extends AbstractLessCssMojo {
    ...
    /**
     * The suffix to be appended to filenames when compress is on.
     *
     * @parameter expression="${lesscss.compressedSuffix}" default-value="-min"
     */
    private String compressedSuffix;
    /**
     * When <code>true</code> the plugin will watch for changes in LESS files
     * and compile if it detects one.
     *
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (getLog().isDebugEnabled()) {
            ...
            getLog().debug("compressedSuffix = " + compressedSuffix);
        }
    ...
    private void compileIfChanged(String[] files, Object lessCompiler) throws MojoExecutionException {
        try {
            for (String filename : files) {
                File input = new File(sourceDirectory, filename);

                buildContext.removeMessages(input);

                if (outputFileFormat != null) {
                    filename = outputFileFormat.replaceAll(FILE_NAME_FORMAT_PARAMETER_REGEX, filename.replace(".less", ""));
                }

                File output = getCompressedFile(outputDirectory, filename);
                ...
                try {
                    ...
                    if (force || !output.exists() || output.lastModified() < lessSource.getLastModifiedIncludingImports()) {
                        ...
                        getLog().info("Compiling LESS source: " + filename + "...");
                        ...
                        getLog().info("Bypassing LESS source: " + filename + " (not modified)");
                    }
                } catch (IOException e) {
                    buildContext.addMessage(input, 0, 0, "Error compiling LESS source", BuildContext.SEVERITY_ERROR, e);
                    throw new MojoExecutionException("Error while compiling LESS source: " + filename, e);
    ...
    private File getCompressedFile(final File outputDirectory, final String originalFile) {
        int extensionIndex = originalFile.lastIndexOf(".");
        String fileName = extensionIndex > 0 ? originalFile.substring(0, extensionIndex) : originalFile;
        String extension = extensionIndex > 0 ? originalFile.substring(extensionIndex) : "";
        if (".less".equalsIgnoreCase(extension)) {
            extension = ".css";
        }

        if (compress && StringUtils.isNotBlank(compressedSuffix) && !(fileName.endsWith(compressedSuffix))) {
